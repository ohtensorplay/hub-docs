...
```bash
mega models list
mega datasets list research/evals --revision main
...
Both upload commands use the Hugging Face-compatible commit workflow. Use the
resumable uploader for large release trees and the regular upload command for
...
See [Discussions and Pull Requests](/docs/hub/discussions) for branch-backed proposals, diff review, fast-forward merge, and moderation commands.
...
Use the [Python SDK](/docs/hub/sdk) for the typed method map and the live [OpenAPI Explorer](/spaces/mega/openapi#tag/Repositories) for direct HTTP integration.
