...
endpoint to find a live `responses` route. Responses is a separate validated
task. A model available for Chat Completions is not automatically available on
this endpoint.
...
Set `stream: true`. MEGA preserves Provider Server-Sent Events, including
output-text delta events. OpenAI-compatible SDKs expose those events through
their normal streaming iterator.
...
Pass `X-Mega-Session-Id` when the same conversation should prefer a stable
upstream account. The Router also recognizes `previous_response_id` as an
affinity signal and forwards it to the Provider.
